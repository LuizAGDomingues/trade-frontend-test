...
export default function Times() {
  ...
  useEffect(() => {
    leagueList?.map((leagues: any) => {
      if(leagues.league.name === league) {
        setSeasonList(leagues.league)
      }
    })
  }, [league])
      ...
      <div className='flex justify-between'>
        <form action="" className='flex flex-col mt-28'>
          ...
          <select name="season" id="season" onChange={e => setSeason(e.target.value)} className='bg-[#D5E2F1] px-3 py-2 rounded-lg w-[308px] mt-1 text-black'>
            <option value=""></option>
            {leagueList?.map((seasons: any) => {
              return (
                <option value={seasons.seasons.year} key={seasons.seasons.year}>{seasons.seasons.year}</option>
              )
            })}
          </select>
