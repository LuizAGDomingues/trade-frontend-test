...
export default function Times() {
  ...
  const [seasonList, setLeagueList] = useState<{}[]>()
    ...
    <div className="w-full h-screen bg-[#223142] text-white px-7">
      ...
      <div className='flex justify-between'>
        <form action="" className='flex flex-col mt-28'>
          ...
          <select name="season" id="season" onChange={e => setLeague(e.target.value)} className='bg-[#D5E2F1] px-3 py-2 rounded-lg w-[308px] mt-1 text-black'>
            <option value=""></option>
            {leagueList?.map((season: any) => {
              return (
                <option value={season.seasons.year} key={season.seasons.year}>{season.seasons.year}</option>
              )
            })}
          </select>
