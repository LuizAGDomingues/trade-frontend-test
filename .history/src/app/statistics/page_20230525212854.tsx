...
function page() {
  ...
  const { data: timeStatisticsData, isLoading: timeStatisticsLoading, error: timeStatisticsError } = useQuery('countries', () => {
  ...
  const { data: timeSquadData, isLoading: timeSquadLoading, error: timeSquadError } = useQuery('countries', () => {
    return api.get('countries', {
      headers: {
        "x-rapidapi-host": API_HOST,
		    "x-rapidapi-key": API_KEY
