...
function TeamsShields(props: TeamShieldsProps) {
  ...
  useEffect(() => {
    if(props.country != '' && props.leagueId != '' && props.season != '') {
      ...
      }).then(res => {setTeams(res.data.response); console.log(teams)})
    }
  }, [props])
  ...
  return (
    <div className='grid grid-cols-4 gap-24 mr-40'>
      {teams?.map((team: any) => {
        return (
          <div key={team.team.id} className="flex flex-col items-center gap-5">
            <Image src={team.team.image} alt="" width={200} height={200}/>
