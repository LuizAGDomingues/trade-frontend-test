...
function TeamsShields(props: TeamShieldsProps) {
  ...
  return (
    <div className='grid grid-cols-4 gap-24 mr-40'>
      {teams?.map((team: any) => {
        return (
          <Link href={`/statistics?league=${leaprops.leagueId}&seasonYears=${seasonYear}&team=${team.team.id}`} onClick={() => handleClick(props, team.team.id)} key={team.team.id} className="flex flex-col items-center justify-between gap-5">
