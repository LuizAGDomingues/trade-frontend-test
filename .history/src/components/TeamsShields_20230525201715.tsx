...
const handleClick = ({leagueId, seasonId}: TeamShieldsProps, teamId: string) => {
  const router = useRouter();
  const league = leagueId;
  const seasonId = season;
  const team = teamId;

  const query = `?info1=${league}&info2=${seasonId}&info3=${team}`;
...
function TeamsShields(props: TeamShieldsProps) {
  ...
  return (
    <div className='grid grid-cols-4 gap-24 mr-40'>
      {teams?.map((team: any) => {
        return (
          <button onClick={() => handleClick(props, team.team.id)} key={team.team.id} className="flex flex-col items-center justify-between gap-5">
