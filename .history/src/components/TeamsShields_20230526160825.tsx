...
function TeamsShields(props: TeamShieldsProps) {
  ...
  const handleClick = ({leagueId, season}: TeamShieldsProps, teamId: string) => {
    ...
    const query = `?info1=${league}&info2=${seasonYear}&info3=${team}`;
  };
  ...
  return (
    <div className='grid grid-cols-4 gap-24 mr-40'>
      {teams?.map((team: any) => {
        return (
          <Link href={`/statistics?info1=${league}&info2=${seasonYear}&info3=${team}`} onClick={() => handleClick(props, team.team.id)} key={team.team.id} className="flex flex-col items-center justify-between gap-5">
