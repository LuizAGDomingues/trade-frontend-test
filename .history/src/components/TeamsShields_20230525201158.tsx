...
const handleClick = () => {
  ...
  const informacao1 = 'valor1';
  const informacao2 = 'valor2';
  const informacao3 = 'valor3';

  const query = `?info1=${informacao1}&info2=${informacao2}&info3=${informacao3}`;
...
function TeamsShields(props: TeamShieldsProps) {
  ...
  return (
    <div className='grid grid-cols-4 gap-24 mr-40'>
      {teams?.map((team: any) => {
        return (
          <button onClick={handleClick} key={team.team.id} className="flex flex-col items-center justify-between gap-5">
